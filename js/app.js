function appViewModel() {
    ...
    function initMap() {
        ...
        // Create the search box and add it to DOM using Map object.
        ...
        // Create the list of places and add it to DOM using Map object.
        var list = document.getElementById('list');
        map.controls[google.maps.ControlPosition.RIGHT_TOP].push(list);

        //Create the show/hide list button and add it to DOM using Map object.
    ...
    function getAllPlaces() {
        var request = {
            location: myLatLng,
            radius: 800,
            types: ['bakery', 'restaurant', 'cafe', 'pharmacy']
        };
    ...
    /* Autocomplete to give application the type-ahead-search behavior of the Google Maps search field.
    When user starts typing an address, autocomplete will fill in the rest.*/
    function autoComplete() {
        var input = document.getElementById('pac-input');
        var searchBox = new google.maps.places.SearchBox(input);
        input.addEventListener("keypress", clearMarkers);
        ...
        searchBox.addListener('places_changed', function() {
            var places = self.search(self.allPlaces);
            if (places.length === 0) {
                return;
            }

            // For each place, create its marker.
